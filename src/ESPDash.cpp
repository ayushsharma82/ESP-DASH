...
CardNames cNames[] = {
    ...
    {TYPE_TABLE_CARD, "table", "updateTableCard"}
};
    ...
    if (type == WS_EVT_DATA)
    {
        ...
        if (info->final && info->index == 0 && info->len == len)
        {
            if (info->opcode == WS_TEXT)
            {
                ...
                {
                    // execute and reference card data struct to function
                    int id = json["id"];
...
// overload function for integer value update
...
void ESPDashV3::UpdateCard(const int cardID, String &value)
{
    ...
    Serial.print("char pointer before: ");
    Serial.println(cData[cardID].value_s);
