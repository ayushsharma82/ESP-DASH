...
dash::Component::Component(ESPDash& dashboard, const char* name, Type type) : _id(nextId()), _name(name), _type(type) {
  dashboard.add(*this);
}
...
#ifdef DASH_DEBUG
      DASH_LOGW("ESPDash", "Component with ID %" PRIu32 " already exists", component.id());
#endif
      return false;
    }
